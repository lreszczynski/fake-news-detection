...
tf_vector = TfidfVectorizer(stop_words=stop_words, max_features=500)
res = tf_vector.fit(df["text"])

print(tf_vector.get_feature_names())
print(len(tf_vector.get_feature_names()))

X_text = tf_vector.transform(df["text"].ravel()).todense()
y_values = np.array(df["target"].ravel())
...
def generate_heatmaps(values):
    ...
    # plt.show()
    plt.savefig("figures/conf_matrix_" + dataset_name + "_bow.png", dpi=200)
    plt.clf()
...
file1 = open("figures/acc_bow_" + dataset_name + ".txt", "w")
file1.writelines("\n".join(accuracy_list))
file1.close()
